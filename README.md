...
**当前版本：V2.5.2 — P0/P1 大批量 bug 修复（26 个）+ B1 云函数鉴权拦截 + dev tools 工具链**
...
- **V2.5.2 P0/P1 修复**：B1 云函数鉴权拦截 + 26 个 bug 修复（前端 UX / 数据一致性 / 算法防御 / 现场发现）+ dev tools 自动注入 `devMode` 工具链 + 学习页步骤指示器 4→3 + 看字说音模式视觉居中
...
| 版本 | 日期 | 说明 |
|------|------|------|
| V2.5.2 | 2026-06-25 | P0/P1 大批量 bug 修复（26 个）+ B1 云函数鉴权拦截 + dev tools 工具链 |
