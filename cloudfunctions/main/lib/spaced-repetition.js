...
/**
 * R-14: 成长等级计算
 * @param {number} masteredCount - 已掌握字数
 * @returns {{level, label, icon, next, progress}} 等级信息
 */
function getGrowthLevel(masteredCount) {
  var levels = [
    ...
    // max 用 Infinity 避免 >9999 时 fallback 错位 (历史 bug: max:9999 让 10000 字算 level 1)
    { level: 5, label: '大树', icon: '🏆', min: 700, max: Infinity, next: null }
  ];

  for (var i = 0; i < levels.length; i++) {
    if (masteredCount >= levels[i].min && masteredCount <= levels[i].max) {
      var progress = levels[i].next
        ? Math.round(((masteredCount - levels[i].min) / (levels[i].max - levels[i].min + 1)) * 100)
        : 100;
      return {
